...
- [ ] **Phase 23.1: EdgeToLineDistance ROI 티칭 배선 + 다점 치수 지원** (INSERTED) — SOP 갭: ROI 측정별 티칭 미배선 + 다점 치수(P1/P2) 미지원 + EdgeSelection 마이그레이션 + Datum 실측검증 + CO-23-01 재검증
...
Plans:
- [ ] 23.1-01-PLAN.md — EdgeSelection 구조적 차단 (TryExecute "All" 고정 + ICustomTypeDescriptor hide, D-08/D-09) — Wave 1
- [ ] 23.1-02-PLAN.md — ROI 측정별 티칭 배선 + 다점 운용 UI (CommitRectRoi Measurement 분기 + GetCurrentFAIRois 다점 렌더, D-01~D-05) — Wave 2
- [ ] 23.1-03-PLAN.md — msbuild 검증 + 실데이터 SIMUL UAT (Phase 23 SC#1~SC#4 통합 sign-off, autonomous: false, D-12~D-15) — Wave 3
...
| 23.1. EdgeToLineDistance ROI 티칭 배선 + 다점 치수 (INSERTED) | 0/3 | ⏳ Planned | - |
