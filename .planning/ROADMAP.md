...
- [ ] **Phase 37: Side 다중 Datum (4 DualImage / 8-image) 검사 구조** (신설 2026-05-28, CO-36-06/07 흡수) — datum 4개(각 VerticalTwoHorizontalDualImage 2장=8장) 각자 별도 Shot + 측정은 별도 이미지 + datum 독립 실행(전부-성공 강제 제거). 검사 실행 흐름 + 데이터모델(datum↔shot↔image 매핑) + UI 전반. **Plans:** 3 plans / 3 waves. 37-01 TryRunDatumPhase lenient; 37-02 EStep.DatumPhase datum-loop + TryRunSingleDatum accumulate; 37-03 UI 4-datum check + SIMUL UAT.
