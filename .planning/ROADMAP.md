...
**Goal:** SIDE 자재 간 측정 편차의 원인은 **금속 띠(기준선)가 휜 것**이다(2026-09-17 실데이터). 지금 기준선은 띠의 두 지점을 이은 직선이라 핀 위치의 실제 띠 높이를 따라가지 못한다. EdgeToLineDistance 에 **측정별 옵션**을 추가해, 측정과 같은 사진에서 **핀 바로 옆 띠**를 기준 ROI 로 찾고 그 선까지 거리를 잰다. 기본 꺼짐.
...
- 기준 ROI 는 측정과 **같은 사진**에서 찾는다
- 못 찾으면 기존 기준선으로 자동 전환 + 로그, 검사는 계속
- 결과 화면·기록에 어느 기준을 썼는지 표시
- Z 범위 자동 선택·레시피 구조·PLC z 번호 불변. F9·다른 검사 확대는 효과 확인 후 별도 phase
