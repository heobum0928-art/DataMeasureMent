...
**Scope**:
  ...
  - Datum 2-image 구조 변경:
    1. `DatumConfig` 다중 이미지 참조 필드 (ImageSourceMode/SourceShotName/TeachingImagePath 다중화)
    2. `TryFindDatum`/`TryTeachDatum` 시그니처 확장 (`IEnumerable<HImage>` 오버로드)
    3. 3개 datum 알고리즘 (TwoLineIntersect/CircleTwoHorizontal/VerticalTwoHorizontal) 이미지 간 피처 융합
    4. `Action_FAIMeasurement.GrabOrLoadDatumImage` 다중 이미지 취득
    5. 단일 이미지 하위호환 (기본 오버로드/팩토리)
