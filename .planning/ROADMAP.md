...
### Phase 999.3: 날짜 범위 CPK 리포트 — 양산 CSV 이력에서 직접 뽑기 — ✅ 구현 완료 quick-260819-ecf (2026-08-19, b9ad5da/a88dbba/5902006)
...
- [x] 68-01-PLAN.md — ZIndexA/ZIndexB 필드(측정+Datum) + Load sentinel 하위호환 + SkipReason.ZINDEX_MISCONFIGURED (D-04/D-05/D-07) [Wave 1]
...
- [x] 68-03-PLAN.md — 크로스-Z 이미지 저장소 + FAI 캡처/주입 + D-08 라이브 fix + 오설정 NG 게이트 (D-02/D-02a/D-03/D-05/D-08) [Wave 3]

**Wave 4** *(blocked on Wave 3 completion)*

- [x] 68-04-PLAN.md — Datum 레벨 크로스-Z 라이브 캡처 + Datum 오설정 게이트 (D-06) [Wave 4]
...
- [x] 68-06-PLAN.md — FIX-0 크로스-Z 사이클 리셋 타이밍: 저장소 clear 를 z=0 응답→z=0 수신 시점으로 이동(BeginCrossZImageCycle) — role A 생존 [Wave 6]

**Wave 7 — GAP-1 + GAP-2 (공유 헬퍼)** *(blocked on Wave 6)*

- [x] 68-07-PLAN.md — GAP-1 선언 z_index 유니버스(Datum ZIndexA/B 포함, 위험 규칙 미채택) + GAP-2 datum-only 실행 스코프(z=0 가드) + DatumPhase 후 Grab/Measure 스킵 [Wave 7]
...
- [x] 68-10-PLAN.md — GAP-3 크로스-Z Datum 즉시-F 재평가 + m_bImmediateFailSent latch(중복-F 방지, z=0 분기 포함) + EnableCrossZDatumImmediateFail 게이팅(기본 OFF, PLC 협의 checkpoint) [Wave 9, autonomous:false]

**Wave 10 — Gap-Closure UAT 재실행** *(blocked on Wave 6~9)*

- [x] 68-11-PLAN.md — gap-closure 통합 SIMUL UAT + 68-05 재실행(→ 68-GAP-UAT.md, 기존 UAT 파일 미수정) + 혼합 Shot 오염 운영 규칙/확인 항목(→ 68-VALIDATION.md) [Wave 10, autonomous:false]
...
### Phase 73: SIDE 시퀀스를 지그 4개(SIDE_1~4)로 분리 — Type 2~5 라우팅 + 지그별 z_index 0부터 재시작 (신설 2026-08-26)

**Goal:** SIDE 를 단일 시퀀스(z=0~15 한 바퀴에 지그 4개)에서 지그별 독립 시퀀스 4개로 분리한다. Top/Bottom 과 동일하게 각 시퀀스 z 가 0부터 시작하고 `$TEST` 의 Type 2~5 가 SIDE_1~4 로 라우팅되어, **지그마다 개별 P/F 판정**이 나가 제어가 지그별 배출을 판단할 수 있게 된다.

**배경:** 제어팀 확정 스펙(2026-08-13)은 Type 을 "검사 대상"(SIDE_1~4=2,3,4,5)으로 정의하나, 현 구현의 `ResourceMap.TryResolveSlotByType` 은 Type 2/4/5 를 Top 슬롯으로 폴백시킬 뿐 SIDE 검사에서 Type 을 실질적으로 무시하고 z_index 만으로 대상을 정한다. 그 결과 ①최종 P/F 가 z=15 에서 한 번만 나와 어느 지그가 불량인지 알 수 없고(4개 중 1개만 NG 여도 통째로 F) ②1번 지그 NG 여도 11초를 다 쓰며 ③"z 로 구분할 거면 Type 이 왜 있나"라는 스펙-구현 불일치가 남는다. 사용자 확인: Datum 하나 = 지그 하나(3-1/3-2/4-2/4-1).

**핵심 이점:** `InspectionSequence` 의 `DATUM_Z_INDEX`(z=0=새 사이클)와 `ComputeLastZIndex()`(`shot.OwnerSequenceName == Name` 으로 자기 것만 집계)가 이미 시퀀스 단위로 동작하므로, 분리만 하면 지그별 사이클/판정이 **기존 엔진 재사용으로 자동 성립**한다.

**Requirements:** 제어 프로토콜 정합 (공식 REQ-ID 미매핑 — 제어팀 스펙 주도)
**Depends on:** Phase 72
**Seed:** `73-SEED.md` (변경 범위 16곳/5파일 grep 실측, 위험 3건, baseline, 코딩규칙, 품질게이트 포함)

**변경 범위(2026-08-26 실측):** `SEQ_SIDE`/`ESequence.Side` 참조 16곳 / 5파일 — `Custom/Define/ID.cs`(enum), `Custom/Sequence/SequenceHandler.cs`(상수·IsSequenceActive·RegisterSequences·RegisterActions·InitializeSequences·TryGetBlockingSequence), `Custom/TcpServer/ResourceMap.cs`(Type 라우팅), `InspectionRecipeManager.cs`, `ShotConfig.cs`, `UI/ContentItem/MainView.xaml.cs`. 4개 시퀀스 모두 `CAMERA_SIDE`+`LIGHT_BAR` 공유(Top/Bottom sharedMil 선례). 레시피는 `OwnerSequenceName` 이 INI 에 그대로 저장돼 있어(35건, SIDE 4건) 스크립트 편집 가능.
...
### Phase 75: Align 정합 검증 — 비전/피커 책임 분리 (신설 2026-08-27)
...
**짝 구조(사용자 확인):** `PC1 = Tray Align + Top/Bottom 검사` / `PC2 = Bottom Align + Side 검사(지그 4개)`. **짝이 같은 PC 안에 있어 로그 병합·PC 간 통신이 불필요**하다 — ①과 ②가 같은 프로세스에서 발생하므로 메모리에서 바로 이어붙인다. ② 는 짝이 되는 검사의 Datum 검출 원점에서 가져오고, **연결 고리는 자재번호**(양쪽 프로토콜이 이미 주고받음).

**신규 구축 최소:** ② 기준점 좌표는 **이미 매 사이클 계산 중**(`DatumConfig.DetectedOriginRow/Col`) — 쓰고 버리는 걸 저장만 하면 된다. ① 은 기존 매칭을 보정 후 한 번 더 호출. **카메라 추가 없음, 택트 증가 없음.**

**범위:** ①재매칭 · ②안착위치 기록 · 자재번호 조인 · 판정 표시(결과 리뷰어에서 자재번호 조회) · 보정 이미지 저장(옵션, 원 요구) · 추세(최근 N개 평균/최대)

**⚠ 알려진 한계:** **Side 는 앞뒤(깊이) 방향을 못 본다** — 측면 촬영이라 카메라 쪽으로 밀려도 그림이 거의 안 변하고 초점만 흐려진다. 사용자 확인상 **Side 는 잡아주는 지그가 없어** 밀릴 여지가 실재하며, 초점이 흐려지면 에지가 뭉개져 **측정값이 조용히 밀린다**(에러 없음). 관련 신호가 이미 로그에 있다(`[FitLine] low strip coverage: ok 8/20`). **이번 범위 밖 — 한계로 기록하고 실제 문제 확인 시 별도 phase(초점/에지 품질 감시)로 다룬다.**

**⚠ 메모리 사고 이력:** 58.3GB 폭증(이미지 큐 무제한) · 34~41GB + halcon.DLL 크래시(Dispose 해도 OS 미반환, 미해결). 이미지 저장을 붙이는 phase 라 **큐 상한·백프레셔·HImage Dispose 필수.**

**Requirements:** AV 계열 — 공식 REQ-ID 미매핑
**Depends on:** Phase 60(ALIGN_CALIB), Phase 65(Bottom 6슬롯), Phase 73(SIDE 지그 4분리 — ② 집계 단위)
**Seed:** `75-SEED.md`
...
### Phase 78: 리뷰어 NG 원인 분석 (규칙 기반 자동 추정 + NG 누적 엑셀)
...
- 화면(NG 행 선택) + 엑셀 둘 다 (D-78-04) · 쉬운 한국어 원인/근거/확인할 일 3줄 (D-78-05)
- 삭제: 차트 이미지 캡처 점검, 사이클 1건 엑셀 export · 유지: 반복검사 묶음, Align 정합 조회 (D-78-06)
- 리뷰어 사진 = 실제 촬영 사진(OriginImageFileName 우선) (D-78-07)
...
4. 삭제 버튼 2개 제거, 나머지 리뷰어 기능 회귀 0
5. 옛 cycle.json 로드 호환
...
### Phase 79: SIDE 핀 옆 띠 기준 옵션 (EdgeToLineDistance 국부 기준선)

**Goal:** SIDE 자재 간 측정 편차의 원인은 **금속 띠(기준선)가 휜 것**이다(2026-09-17 실데이터). 지금 기준선은 띠의 두 지점을 이은 직선이라 핀 위치의 실제 띠 높이를 따라가지 못한다. EdgeToLineDistance 에 **측정별 옵션**을 추가해, 기준점 가로 사진(z1)에서 **핀 바로 옆 띠**를 기준 ROI 로 찾고, 측정 사진의 핀 에지에서 그 선까지 거리를 잰다. 기본 꺼짐.

**왜 가능한가 (데이터로 확인, 2026-09-17 SIDE_1 자재 A·B)**

| 기준 | 자재 A−B 차이 |
|---|---|
| 지금 (띠 두 지점을 이은 직선) | C13_P3 −48µm · F9 −65~−81µm |
| 핀 바로 옆 띠 | 왼쪽 +2.8µm · 오른쪽 +4.0µm (반복 흔들림 ≤2.3µm) |
...
- 못 찾으면 기존 기준선으로 자동 전환 + 로그, 검사는 계속
- 결과 화면·기록에 어느 기준을 썼는지 표시
- 옵션은 SIDE·TOP·BOTTOM 의 모든 EdgeToLineDistance 측정에서 레시피 티칭만으로 켤 수 있게 만든다 (기준 ROI = 그 측정의 기준점이 가로선을 찾은 사진). 이번에 켜서 효과를 검증하는 대상은 C13·C14 6점, TOP·BOTTOM 은 동작·폴백 확인까지
...
### Phase 80: 리뷰어 NG 사이클 사진 한 번에 불러와 파라미터 수정·재검사

**Goal:** 리뷰어에서 NG 로 확인한 검사를 고르면, 그 검사에 쓰인 사진 전부(해당 측정 Shot 사진 + 그 측정이 쓰는 기준점 가로·세로 사진 + 같은 자재 검사에서 찍은 다른 Shot 사진)를 한 번에 메인 화면으로 불러와, 파라미터를 고치고 바로 재검사할 수 있게 한다. 지금은 기준점 사진을 따로 불러와야 하고 어떤 사진을 불러야 할지 찾기 불편하다 (사용자 2026-09-18: "결국은 리뷰어에서 NG 이미지로 확인된 거는 내가 파라미터 수정을 할 때 보다 쉽게 할 수 있게 해당 이미지를 열 수 있게 하는 게 관건").

**함께 처리:** 리뷰어 사이클 전체 보기에서 여러 Shot 의 검출 선이 사진 한 장에 겹쳐 그려지는 버그(`ReviewerWindow.xaml.cs:183`, 2026-09-18 확인 — 같은 사진·선 짝 맞추기가 이 기능의 전제)

**함께 처리 (사용자 2026-09-18 요청):** 수동 RUN 에서 기준 ROI(Local Ref)를 고친 뒤 Test Find 없이 RUN 만 눌러도 새 기준 ROI 로 국부 기준선을 다시 계산 — 지금은 기준점 찾을 때 한 번 계산해 둔 값을 쓰므로 ROI 를 고치면 "설정이 바뀜" 으로 전역 전환되고 Test Find 를 다시 눌러야 반영된다(Phase 79 UAT U-3 에서 확인). PLC 자동 검사는 매 사이클 기준점을 새로 찾으므로 영향 없음.

**확정 방향 (사용자 2026-09-18):** 리뷰어에서 고른 Shot 사진을 불러올 때 **그 Shot 이 쓰는 기준점(Datum) 사진도 자동으로 같이 불러오고 Test Find 까지 자동 실행** — 따로 불러오고 맞추는 게 불편하고 헷갈림. 짝이 맞는 기준점 사진이 없으면(안 찍혔거나 파일 없음) 수동 검사 때 **다이얼로그로 알림**. 불러온 사진·선은 리뷰어에서 본 것과 똑같아야 한다.

**후보 (논의 때 결정):** 기준 ROI 티칭 시 z1 기준점 사진을 배경으로 상자를 보여 주고 바로 에지를 찾아 보는 "시험 찾기" (Phase 79 UAT 중 "측정 사진에서는 에지가 안 보임" 불편)
