...
Last activity: 2026-06-19 -- Quick 260619-cnm: per-shot 측정 보정계수 CorrectionFactor 백엔드(공칭=CAD 확정 → ~0.5% 캘리브 간극 보정 layer). PixelResolution 고정 불변 + 별도 곱셈 보정계수(CameraSlaveParam, GetEffectivePixelResolution 단일소스, 기본1.0 회귀0, 가드레일 ±2%). 에이전트 패널 per-shot 합의. 컴파일 0 errors(d6c95a7). 다음=SIMUL UAT(0.995 적용→공칭근처 확인)
