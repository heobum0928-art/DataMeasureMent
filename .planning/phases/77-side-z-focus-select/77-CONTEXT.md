...
| D-77-08 | **범위 변경 시 항상 다이얼로그 알림 (2026-09-15, 사용자: "디폴트는 0으로 해 놓고 범위 바꿀때 다이얼로그로 알림도 넣어놔").** 기본값 `ZIndexEnd = 0`(꺼짐, 1장) 유지. PropertyGrid 에서 **"Z 범위 끝"(또는 범위가 켜진 Shot 의 `ZIndex`)을 바꿀 때마다** 다이얼로그를 띄운다: ① 정상 범위 → 정보 다이얼로그 "Z 범위 z4~z6 (3장) — 측정마다 가장 선명한 Z 자동 선택, PLC 가 4~6 번호로 차례로 촬영해야 동작" ② 0 으로 바꿈 → 정보 다이얼로그 "Z 범위 꺼짐 — ZIndex(4) 사진 1장으로 측정" ③ 오설정·겹침 → 기존 경고 다이얼로그 + "한 장만 쓰려면 0" 안내. INI 로드·붙여넣기에서는 뜨지 않는다(기존 억제 유지), 저장은 막지 않는다. 구현 시점: 77-04 완료 후 77-05 전 소규모 작업. |
...
- **D-77-01:** PLC 가 Z 높이마다 새 z 번호로 $PREP/$TEST 를 보낸다. 앱은 Z 축을 움직이지 않는다.
- **D-77-02:** 선명도 점수는 측정이 이미 계산하는 measure_pos 에지 강도(|amp|)로 한다. sobel_amp 별도 계산 안 함.
- **D-77-03:** Shot 단위 Z 범위 + 그 Shot 의 측정(ROI)마다 자동 선택.
- **D-77-04:** 사용자 원래 표현 "같은 shot 에서 ROI 내 에지들의 평균값으로 가장 강한 에지로 선택" 을 점수 정의의 기준으로 삼는다.
- **D-77-05:** strip 강도 = strip 안 가장 강한 |amp| 1개, 점수 = Σ(strip 최대 |amp|) ÷ EdgeSampleCount, 에지 없는 strip = 0.
- **D-77-06:** 수동 라이브(RUN·수동 트리거)는 1장 측정+안내, 오프라인·저장사진 재검사는 z 별 사진 있으면 같은 선택 로직(없으면 1장+경고), 후보 z 사진 저장 체크박스 기본 꺼짐.
- **D-77-07:** 운영 최소 입력 — Shot 에 ZIndexEnd 1개만(ZIndexStart 없음), 측정별 설정 없음, 동점 허용치 3% 내부 const, 선택 Z 한 칸 표시, 한국어 한 줄 경고.
- **D-77-08:** 기본값 ZIndexEnd=0 유지, Z 범위 끝(또는 범위 켜진 Shot 의 ZIndex) 편집 시 항상 다이얼로그 — 정상 범위는 정보(zA~zB, N장), 0 은 정보(꺼짐·1장), 오설정·겹침은 경고 + "한 장만 쓰려면 0". 로드·붙여넣기 억제 유지.
</decisions>
