...
**상태:** 사용자 확인 완료 (2026-09-18) — U-1~U-6 PASS, U-7 대기(장비 PC), U-8 미실행(선택 항목). 사용자 승인: "ㅇㅋ" (U-6 기준값·공차 미정, U-7/U-8 은 후속으로 남김을 포함해 승인).

**실행 환경 실측:** Debug 빌드는 이 PC 에 라이선스 키가 없어 실행 불가 — 사용자가 15:48 에 직접 Release 1.7.50.0 을 빌드해 배포 exe(운영 D:\Data)로 UAT 를 수행함(계획의 "Debug|x64 전용" 전제와 다르게 실행됨, 아래 배포 참고 항목 확인).
...
- [x] U-1. Local Ref 탭 UI·티칭 방식 (A-79-E1) — PASS
...
**A-79-E1 판단 칸(자동 해소 금지):** Local Ref 탭 3그룹·숫자 입력·재선택 시 ROI 상자 표시·끌기/크기조정 1~7 모두 확인됨(C13_P3, 추천 stripR 값). 다만 **티칭 중 캔버스가 측정 사진(z3~z9)을 보여주고 z1 기준점 가로 사진을 보여주지 않아, 기준 ROI 를 끌어 맞출 때 정작 에지(띠)가 보이지 않는다** — 숫자 입력만으로는 맞는지 확인할 수 없고 사진을 봐야 확신이 선다. **후속(Phase 80 후보)으로 이동**: "기준 ROI 시험 찾기" 를 z1 사진 위에서 하도록(캔버스에 z1 사진을 띄우거나 미리보기 추가) — 커밋 08623973 에 후보로 기록됨. 그 외 숫자 입력 자체는 쓸 만함.

결과: PASS — 1~7 전부 확인(레시피 저장 후 `C13_P3` 섹션에만 13개 키 신규 생성, `C14_P3` 등 다른 섹션 무변경 확인). A-79-E1 후속 이슈는 위 판단 칸 참고, Phase 80 으로 이월.

---

- [x] U-2. 실제 사이클 — C13_P3 만 켬 (tracer 화면 증거, A-79-E2) — PASS
...
**A-79-E2 판단 칸:** 실측(수동 RUN, 오프라인 사진, PLC TCP 경유 아님)에서 3회 연속 재현 — Algorithm 로그 `[LocalRef] 기준선 찾음 — SIDE_1 · Side_Datum_1 · C13_P3: 중점 row=6476.79 col=12696.47 에지세기=65.7` (3회 row 6476.4~6477.1, 에지세기 65.7~69.1 — 흔들림 작음). `C13_P3` "기준" = 국부, 주황 선이 띠 위, cycle.json `RefSource="Local"`, `C14_P3` 는 빈칸. 전제(z1 사진 좌표를 그대로 쓴다) 는 실제 사이클에서도 성립함이 확인됨.

결과: PASS — 2~8 확인. 자재별 확인(같은 사이클에서 관찰): 자재 A 기준점 origin/angle (6585.8, -0.380°) 대 자재 B (6608.2, -0.507°), 국부 띠 row A 6476.8 대 B 6490.0 로 자재마다 제 사진·제 값을 씀(회귀 없음).

---

- [x] U-3. 전역 기준선 자동 전환 (A-79-E3) — PASS
...
**A-79-E3 판단 칸(자동 해소 금지):** 사용자가 실제 확인한 두 경우 — (a) `LocalRef_Length1=0` (b) `LocalRef_Row=6000`(평평한 위치) — 둘 다 값이 옵션 끈 값과 정확히 같음(2.2078), "기준" 칸 `국부실패→전역`, 사이클 정상 완료로 3·4 는 확실히 통과. 다만 두 경우 모두 Error 로그 원인 문구가 (a)"기준 ROI 가 티칭되지 않음"·(b)"에지 못 찾음" 이 아니라 **`기준점을 찾은 뒤 기준 ROI 또는 에지 설정이 바뀜 — 기준점을 다시 찾으면 반영됨`** 로 찍혔다 — 수동 RUN 이 이전 Test Find 로 잡아 둔 기준점을 재사용하는 구조라 "설정이 바뀜" 검사가 먼저 걸린 것(A-79-A6 과 같은 안전한 동작, 문구 자체는 잘못이 아님). 사용자는 이 흐름에서 **"기준 ROI 를 바꾸면 Test Find 를 다시 누르지 않아도 수동 RUN 이 바로 반영해 주면 좋겠다"** 는 요청을 했고, 이는 코드 변경(아키텍처적 판단 필요)이라 Phase 80 범위로 옮김(커밋 08623973 "수동 RUN 기준 ROI 변경 즉시 반영 항목 추가"). 원인 문구 자체(무엇이 왜 바뀌었는지)는 원인 찾기에 충분하다고 판단.

결과: PASS — 1(a)·2(b) 두 경우 확인(3 번째 경우인 "Test Find 후 ROI 만 변경" 은 (a)(b) 재현 중 자연히 겹쳐 확인됨), 3·4 모두 확인. A-79-E3 후속 요청은 Phase 80 이월.

---

- [x] U-4. 표시·기록 회귀 + 저장 사진 색 (A-79-A4, A-79-A7) — PASS
...
**A-79-A4 판단 칸:** 옵션 꺼진 측정의 "기준" 칸 빈칸 표시 확인 — 값·판정이 옵션 추가 전과 동일. 괜찮음.
**A-79-A7 판단 칸:** 이번 UAT 세션에서 저장 캡처 사진 색 자체에 대한 별도 이견 없음(괜찮음으로 처리) — 후속 quick 필요 시 별도 제기.

결과: PASS — 옵션 꺼짐 → "기준" 칸 빈칸, 값이 옵션 추가 전과 동일함을 확인.

---

- [x] U-5. 켤 측정·창 결정 (O-79-10, A-79-A2, D-79-02) — 결정 완료
...
| C13_P2 | 끔 | - |
| C14_P2 | 끔 | - |
| C13_P3 | **켬** | stripR (추천 그대로) |
| C14_P3 | **켬** | stripR2 (추천 그대로) |

결정 사유: C13_P3·C14_P3 만 국부 기준을 켬(둘 다 추천 창 그대로 사용). P1(C13_P1/C14_P1) 은 전역에서도 이미 편차가 작아(±3.3, -0.5µm) 켜지 않음. P2(C13_P2/C14_P2) 는 찾음 7/7 이지만 A−B 가 국부에서 오히려 악화되어(A-79-A2) 켜지 않음.

결과: 결정 완료 — 위 표대로 C13_P3·C14_P3 만 켬.

---

- [x] U-6. 효과 확인 + 기준값·공차 결정 (A-79-E4, PR-5) — PASS(C13_P3), C14_P3 는 사용자 판단으로 미측정
...
같은 z3/z1 사진 쌍(A1: z3 `origin_..._M1_173857758.jpg` + z1 `datum_..._H_173855718.jpg`, B1: z3 `origin_..._M2_174315651.jpg` + z1 `datum_..._H_174313615.jpg`)으로 재검사한 실측값:

| 측정 | 전역 A−B(µm) | 국부 A−B(µm) | 판정(개선/유지/악화) |
|---|---|---|---|
| C13_P3 | 48.7 (off: A=2.2565, B=2.2078) | **8.4** (on: A=2.1811, B=2.1727) | **개선** — realab 예측(47.9→9.5)과 일치 |
| C14_P3 | (미측정) | (미측정) | 사용자가 "C14 도 동일하겠지" 로 판단해 실측 생략 — realab 예측(47.3→2.7) 을 참고값으로 남김, 다음 재검사 시 실측 권고 |
...
|---|---|---|---|
| C13_P3 | **미정** — 도면·고객 확인 후 결정 예정 | 미정 | 미정 |
| C14_P3 | 미정(C13_P3 결정과 함께 논의) | 미정 | 미정 |

참고: 국부 기준(옵션 ON) 상태에서 C13_P3 ≈ 2.177mm, 현재 기준값 2.05 ±0.03 대비 여전히 NG 범위(+0.13mm 남는 편차는 이번 phase(자재 간 흔들림) 와 별개인 전체 바이어스 이슈 — 도면·기준값 재설정으로 다뤄야 함).

합격 기준: 1~4 모두 기록(C14_P3 실측은 사용자 판단으로 생략, 4번 기준값·공차는 결정 보류).

**A-79-E4 판단 칸(자동 해소 금지):** C13_P3 는 realab 예측(47.9→9.5µm)과 실측(48.7→8.4µm)이 거의 일치 — **뚜렷이 줄었다고 판단**. C14_P3 는 미실측이나 realab 예측(47.3→2.7µm)이 비슷한 개선 폭을 보여 같은 결론으로 추정. 종합: **Phase 79 효과 인정, 승인**. 다만 기준값(NominalValue)·공차는 이번 UAT 범위에서 결정하지 않고 도면/고객 확인 후로 미룸.

결과: PASS(C13_P3 실측 확인) — U-6 은 "승인" 으로 종료하되, C14_P3 미실측·기준값 미정 2건은 알려진 잔여 항목으로 아래 최종 결론에 남긴다.
...
- [ ] U-8. 동시 작업 안전성 (edge LSR-04 concurrency) — 대기(선택 항목, 미실행)
...
결과: 대기(미실행) — 선택 항목(backstop). U-1~U-6 PASS 로 사용자 승인은 이미 이뤄졌으므로 별도로 실행하지 않음. 필요 시 후속으로 수행.
...
10. **C13_P1 추천 창 주의:** 추천 규칙(찾음 7/7 + 흔들림 최소)이 고른 stripL2 는 실제로는 형제 창 stripL 보다 A−B 가 훨씬 나쁘다(39.4 vs 3.3µm) — U-5 에서 stripL 사용을 검토할 것. (U-5 에서 P1 은 켜지 않기로 결정되어 이번 phase 에서는 실사용 영향 없음.)
11. **U-1 티칭 중 z1 사진 미표시 (신규, A-79-E1):** 기준 ROI 를 끌어 맞출 때 캔버스가 z3~z9 측정 사진을 보여주고 z1 기준점 가로 사진(에지가 실제로 피팅되는 사진)을 보여주지 않아 숫자만으로 맞춰야 한다 — Phase 80 후보("기준 ROI 시험 찾기")로 이월(커밋 08623973).
12. **U-3 수동 RUN 재검출 지연 (신규, A-79-E3):** 기준 ROI 를 바꾼 뒤 Test Find 를 다시 누르지 않으면 수동 RUN 이 이전 기준점을 재사용해 전역으로 전환된다(설계상 안전 동작이나 운영 편의상 즉시 반영을 원함) — Phase 80 후보로 이월(커밋 08623973).
13. **U-6 C14_P3 미실측·기준값 미정:** C14_P3 는 사용자가 "C13_P3 와 동일할 것" 으로 판단해 실측을 생략했다 — realab 예측(47.3→2.7µm)만 참고값으로 남아 있고, C13_P3·C14_P3 모두 새 `NominalValue`/공차는 도면·고객 확인 후 별도 결정 예정이다. 이 두 항목은 이번 phase 승인의 조건이 아니라(U-6 A-79-E4 판단에서 이미 개선 인정) 후속 운영 준비 작업으로 남는다.
14. **배포 경로 확인 필요:** 이번 UAT 는 계획의 "Debug|x64 전용" 전제와 다르게, 이 PC 에 라이선스 키가 없어 사용자가 직접 빌드한 Release 1.7.50.0(운영 D:\Data 배포 exe)으로 수행되었다 — UAT 자체는 실제 운영 경로로 수행되어 결과 신뢰도는 오히려 높으나, T-79-19(운영 레시피·배포 exe 보호) 관점에서 이 UAT 세션 중 배포 exe 가 실제로 갱신되었다는 뜻이므로, 운영 레시피(main.ini)에 C13_P3/C14_P3 Local Ref 값을 실제로 적용할지는 U-5 결정과 별개로 사용자가 최종 승인해야 한다(P-15, 이미 안내됨).

---

## 최종 결론

**사용자 승인: "ㅇㅋ"** — Phase 79 (핀 옆 띠 국부 기준) 승인. U-1~U-6 전부 PASS(U-6 은 A-79-E4 개선 인정, 기준값·공차 결정은 별도 이월), U-7(TOP·BOTTOM, 장비 PC) 대기, U-8(동시성, 선택) 미실행.

- 켤 측정: `C13_P3`(stripR), `C14_P3`(stripR2) — U-5 결정 표대로.
- 켜지 않음: `C13_P1`, `C14_P1`, `C13_P2`, `C14_P2`.
- 이월 항목(Phase 80 후보, 이미 커밋 08623973 에 기록): (1) 기준 ROI 시험 찾기를 z1 사진 위에서 하도록(A-79-E1), (2) 수동 RUN 에서 기준 ROI 변경을 Test Find 없이 즉시 반영(A-79-E3).
- 남은 사용자 작업(이번 phase 범위 밖): C14_P3 실측 확인, C13_P3·C14_P3 새 기준값·공차 결정(도면/고객 확인 후), U-7(장비 PC), 운영 레시피(main.ini) 적용 여부 최종 결정.
