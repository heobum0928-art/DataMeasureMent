...
> **UAT 1차 FAIL → 핫픽스 20c9b6f (2026-06-19)**: "기본값인데 전 측정값 0". 원인 = `ParamBase.Load` 가 누락 INI 키를 `ToDouble()=0` 으로 덮어써 `CorrectionFactor` 초기값 1.0 을 0 으로 클로버 → `GetEffectivePixelResolution=PixelResolution×0=0`. 수정 = ① `CameraSlaveParam.Load` 오버라이드서 키 부재 시 1.0 복원 ② `GetEffectivePixelResolution` 에 ≤0 → 1.0 안전클램프. 전역 ParamBase 미변경(타 param 회귀 0). 패턴 = memory reference_parambase_missing_key_zeroes_default. **재빌드 후 UAT PASS (2026-06-19): 기본값 측정 정상복귀(회귀0) + CorrectionFactor 0.995 적용 시 측정값 보정 확인. signed_off.**
