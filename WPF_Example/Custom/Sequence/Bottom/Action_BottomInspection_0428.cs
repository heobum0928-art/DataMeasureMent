...
namespace ReringProject.Sequence {
    ...
    public class BottomInspectionContext : ActionContext {
        ...
        public double Model_ArcLength {get; set;}   // 04.25 Insert 
        public double Min_ArcLength { get; set; }   // 04.25 Insert
        public double Particle_Size { get; set; }   // 04.25 Insert
        ...
        public override void Clear() {
            ...
            Model_ArcLength = 0;    // 04.25 Insert
            Min_ArcLength = 0;      // 04.25 Insert
            Particle_Size = 0;      // 04.25 Insert
    ...
    public class BottomInspectionParam : CameraSlaveParam {
        ...
        [DisplayName("Paticle Size(Pixel)")]
        public double ParticleSize
        {
            ...
            {
                _ParticleSize = value;
                RaisePropertyChanged("ParticleSize");
            }
        }
        private double _ParticleSize;

        [DisplayName("Use Morphology(Dilate)")]
        ...
        public override bool CopyTo(ParamBase param) {
            ...
            if (param is BottomInspectionParam)
            {
                ...
                BIP.ParticleSize = ParticleSize;    // Insert 04.26
    ...
    class ImageProcessor
    {
        ...
        public bool ProcessImage(int ImageGrabIndex, Mat ImageBuffer, BottomInspectionContext pMyContext, BottomInspectionParam pMyParam)
        {
            ...
            if(pMyParam.DebugImgSave == true)
            {
                ...
                string MaskImgFileName = Path.Combine(MaskImgPath, String.Format($"{DateTime.Now.Hour}_{DateTime.Now.Minute}_{DateTime.Now.Second}_{ImageGrabIndex}_Masking.png", ImageGrabIndex));
            ...
            if (pMyParam.DebugImgSave == true)
            {
                ...
                string BinaryImgFileName = Path.Combine(BinaryImgPath, String.Format($"{DateTime.Now.Hour}_{DateTime.Now.Minute}_{DateTime.Now.Second}_{ImageGrabIndex}_Binary.png", ImageGrabIndex));
            ...
            #region Contour Start (Default ROI)
            ...
            temp.ContourArea = Cv2.ContourArea(contours[maxIndex]);   // pixel
            //temp.ContourArea = Cv2.ContourArea(contours[maxIndex])/pMyParam.PixelToUM_Offset;
            ...
            if (pMyParam.CalSelector == BottomInspectionParam.CalibrationBase.Picker)
            {
                ...
                // 04.27 Base 좌표를 Display Picker Number 에 기재된 번호에 해당하는 Base 좌표 값을 표기.
                pMyParam.Xbase = pMyContext.X_Picker_CalValue[pMyParam.Display_Picker_Num];
                pMyParam.Ybase = pMyContext.Y_Picker_CalValue[pMyParam.Display_Picker_Num];
                ...
                ((calAngle < pMyParam.Tolearance_Angle) && (calAngle > -pMyParam.Tolearance_Angle)))
            {
                ...
                /*
                // 04.25 Die Width & Height Ratio 영역을 넓혀 Canny Edge 를 통해 깨짐 또는 Crack 검출을 하기 위해 주석 처리하고, 아래에 임의 값(0.98)을 넣어 Inner ROI를 잡음.
                ...
                */
                ...
                double X1 = (objectCenter_X - ((pMyParam.Die_Width / pMyParam.PixelToUM_Offset) * 0.98) / 2);
                double X2 = (objectCenter_X + ((pMyParam.Die_Width / pMyParam.PixelToUM_Offset) * 0.98) / 2);
                double Y1 = (objectCenter_Y - ((pMyParam.Die_Height / pMyParam.PixelToUM_Offset) * 0.98) / 2);
                double Y2 = (objectCenter_Y + ((pMyParam.Die_Height / pMyParam.PixelToUM_Offset) * 0.98) / 2);
                ...
                if (pMyParam.DebugImgSave == true)
                {
                    ...
                    string DieInnerImgFileName = Path.Combine(DieInnerImgPath, 
                        String.Format($"{DateTime.Now.Hour}_{DateTime.Now.Minute}_{DateTime.Now.Second}_{ImageGrabIndex}_InnerRect.png", ImageGrabIndex));
                ...
                */
                ...
                Cv2.Canny(InnerImg, InnerImg, pMyParam.CannyThresholdLow, pMyParam.CannyThresholdHigh, 3, true);
                //Cv2.ImWrite("./CannyEdge.bmp", InnerImg);

                #region MorphologyEx Start
                /*
                ...
                */
                ...
                // Cv2.DrawContours(CannyContourImg, Cannycontours, -1, new Scalar(0, 0, 255), 2, LineTypes.AntiAlias, null, 1);    // 04.25 주석 처리 -> Particle_Contours 에서 처리.
                //Cv2.ImWrite("./CannyEdgecontour.bmp", CannyContourImg);
                // 03.14 Insert End

                if(Cannycontours.Length != 0)
                {
                    double particle_length = 0.0;
                    double particle_area = 0.0;
                    double particle_max_value = 0.0;

                    List<OpenCvSharp.Point[]> Particle_contours = new List<OpenCvSharp.Point[]>();

                    foreach (OpenCvSharp.Point[] p in Cannycontours)
                    {
                        particle_length = Cv2.ArcLength(p, false);      // Particle 윤곽선의 길이.
                        particle_area = Cv2.ContourArea(p, false);      // Particle 면적.

                        // 04.25 Particle Size Insert
                        if (particle_length > pMyParam.ParticleSize)  // Particle의 비교 대상이 윤곽선의 길이
                        //if (particle_area > pMyParam.ParticleSize)      // Particle의 비교 대상이 면적
                        {
                            Particle_contours.Add(p);
                            //break;
                            if (particle_length > particle_max_value)
                            //if (particle_area > particle_max_value)
                                particle_max_value = particle_length;
                                //particle_max_value = particle_area;
                        }
                    }

                    if(Particle_contours.Count != 0)
                    {
                        Cv2.DrawContours(CannyContourImg, Particle_contours, -1, new Scalar(0, 0, 255), 2, LineTypes.AntiAlias, null, 1);

                        temp.Judgment = false;

                        //Cv2.PutText(CannyContourImg, "Fail - Crack OR Broken CNT:" + Cannycontours.Length.ToString(), new OpenCvSharp.Point(CannyContourImg.Width / 4, CannyContourImg.Height / 2),
                        //Cv2.PutText(CannyContourImg, "Fail - Crack OR Broken length:" + particle_length.ToString(), new OpenCvSharp.Point(CannyContourImg.Width / 4, CannyContourImg.Height / 2),
                        Cv2.PutText(CannyContourImg, "Fail - Crack OR Broken length:" + particle_max_value.ToString(), new OpenCvSharp.Point(CannyContourImg.Width / 4, CannyContourImg.Height / 2),
                        //Cv2.PutText(CannyContourImg, "Fail - Crack OR Broken Area:" + particle_area.ToString(), new OpenCvSharp.Point(CannyContourImg.Width / 4, CannyContourImg.Height / 2),
                        HersheyFonts.HersheyComplex, 1, Scalar.Red, 1, LineTypes.AntiAlias);
                ...
                #region OLD_Canny Predicted Start
                /*
                if (Cannycontours.Length == 0)
                {
                    temp.Judgment = true;
                ...
                */
                #endregion OLD_Canny Predicted End
                ...
                if (pMyParam.CrobImg == true)
                {
                    ...
                    if (Directory.Exists(strMapPath) == false)
                        Directory.CreateDirectory(strMapPath);
                    string strFileName = Path.Combine(strMapPath, String.Format($"{DateTime.Now.Hour}_{DateTime.Now.Minute}_{DateTime.Now.Second}_{ImageGrabIndex}Image.png", ImageGrabIndex));
            ...
            else
            {
                ...
                // 04.25 Insert Start
                string BadPointStr = null;

                if (contours.Length != 1)
                    BadPointStr = "Judgment: BAD => Contour Count Fail";
                else if (convex != false)
                    BadPointStr = "Judgment:BAD => Polygon Fail";
                else if(approx.Length != 4)
                    BadPointStr = "Judgment:BAD => Apex Count Fail";
                else if((calAngle > pMyParam.Tolearance_Angle) && (calAngle < -pMyParam.Tolearance_Angle))
                    BadPointStr = "Judgment:BAD => Angle Fail";
                else
                    BadPointStr = "Judgment:BAD => Area Fail";
                // 04.25 Insert End

                //Cv2.PutText(DrawImg, "Judgment: BAD", new OpenCvSharp.Point(objectCenter_X + 20, objectCenter_Y + 100),
                Cv2.PutText(DrawImg, BadPointStr, new OpenCvSharp.Point(objectCenter_X - 300, objectCenter_Y + 120),
                    HersheyFonts.HersheyComplex, 1, Scalar.Red, 1, LineTypes.AntiAlias);
    ...
    public class BottomInspectionAction : ActionBase {
        ...
        public override void OnBegin(SequenceContext prevResult = null) {
            ...
            if (pMyContext.SeqName == "FRONT_BOTTOM")
            {
                ...
                //BottomCalibrationContext FCalContext = SystemHandler.Handle.Sequences[ParentID][EAction.FrontBottom_Calibration].Context as BottomCalibrationContext;

                pMyParam.Xbase =  pMyContext.Cal_XBase = FCalParam.BaseX;
                pMyParam.Ybase =  pMyContext.Cal_YBase = FCalParam.BaseY;

                // 04.27 Bottom Calibration UI에 Pixel 로 선택되어 있지 않아도, Pixel 값을 가져와 사용하게 수정함.
                for (int i = 0; i < 10; i++)
                {
                ...
                #region OLD Front Calibration Get Value Start
                /*
                // 04.27 이전에는 BottomCalibration의 Param에 표기된 픽셀 값을 그대로 Inspection에 가져와 
                // Calibration Base가 Picker 로 선택된 경우 사용.
                ...
                */
                #endregion OLD Calibration Get Value End
            ...
            else if (pMyContext.SeqName == "REAR_BOTTOM")
            {
                ...
                //BottomCalibrationContext RCalContext = SystemHandler.Handle.Sequences[ParentID][EAction.RearBottom_Calibration].Context as BottomCalibrationContext;
                ...
                #region OLD Rear Calibration Get Value Start
                /*
                ...
                */
                #endregion OLD Rear Calibration Get Value Start
            ...
            // 04.25 Insert start
            pMyContext.Model_ArcLength = (pMyParam.Die_Width / pMyParam.PixelToUM_Offset) * 2 + (pMyParam.Die_Height / pMyParam.PixelToUM_Offset) * 2;
            pMyContext.Min_ArcLength = pMyContext.Model_ArcLength * 0.2;

            Debug.WriteLine($"Min ArcLength:{pMyContext.Min_ArcLength}");
            // 04.25 Insert End 
