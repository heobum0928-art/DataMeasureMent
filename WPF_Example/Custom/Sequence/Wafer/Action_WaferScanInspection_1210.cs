...
namespace ReringProject.Sequence
{
    ...
    public class WaferScanInspectionAction : ActionBase
    {
        ...
        //private Mat BinaryImage = null; // 08.12 Insert

        private int result = -1;

        // 09.27 Insert - Measurement Lap Time 
        ...
        public bool[,] bMatrixExist = new bool[100, 100];   // Insert 12.09
        ...
        public override void OnBegin(SequenceContext prevResult = null)
        {
            ...
            // Insert 12.09 initialize
            for (int i = 0; i < 100; i++)
            {
                for (int j = 0; j < 100; j++)
                {
                    bMatrixExist[i, j] = false;
                }
            }
        ...
        private List<Data> ReadMapFile(string mapFile)
        {
            ...
            OriginData.dSizeX = pMyParam.DieWidth + (pMyParam.Scrib_X * (pMyContext.MMPerPixel_X * 1000));  // Scrib Line 을 포함한 Die X Size
            OriginData.dSizeY = pMyParam.DieHeight + (pMyParam.Scrib_Y * (pMyContext.MMPerPixel_Y * 1000)); // Scrib Line 을 포함한 Die Y Size
        ...
        /// </summary>
        /// Center Position 값 구하는 개수 변경. 원본은 +1 이 없었으나, 행과 열의 개수를 헤깔리지 않게 하기 위해 통일하여
        /// 다이센터인지 스크라이브 센터인지를 확인하게 변경 함. ( CTST에서 출하 후 수정함.)
        /// <returns></returns>
            ...
            try
            {
                ...
                if(pMyParam.InspectionROI == WaferScanInspectionParam.Die_ROI.InnerROI)
                {
                    ...
                    if(ProcessInspection(dieRegion, MPoint, ref TempImage, ref TempContourCount, ref TempContourArea, ref TempConvex, ref TempApexCount, ref TempMoments) == true)
                    {
                        ...
                        //if ((pMyParam.MapCheck == false) && (ContourCount == 1) && (ContourArea >= (roi.Width * roi.Height * 0.7)) && (Convex == false) && (ApexCount == 4)) // Delete new_contours.Count  11.25
                        ...
                        // 07.12 insert : Map File 없는 경우 무조건 true;
        ...
        {
            //if (MatrixPoint.X == 14 && MatrixPoint.Y == 52)
            //    Debug.WriteLine($"Debug-{MatrixPoint}");
            Mat binary = new Mat();
        ...
        /// <summary>
        /// Wafer Angle 에 따른 모델의 Rotation Matrix Position valuel
        /// </summary>
        ...
        private void MapSearch()
        {
            ...
            for (int i = 0; i < ModelResult.nCnt; i++)
            {
                ...
                /*
                if (MatrixPointX == 14 && MatrixPointY == 52)
                    Debug.WriteLine($"Debug-{MatrixPointX}{MatrixPointY}, OX-{oSearchX}, OY-{oSearchY}");

                try
                {
                    if (bMatrixExist[MatrixPointX, MatrixPointY] == false)
                        bMatrixExist[MatrixPointX, MatrixPointY] = true;
                    else
                        continue;
                }
                catch (Exception ex)
                {
                    Debug.WriteLine($"index:{i}, X:{MatrixPointX}, Y:{MatrixPointY}");
                }
                */
                ...
                if (Status != 1)    // Change 04.12
                {
                    ...
                    if (VM_COLS.ContainsKey(MatrixPointX) == false)     // MatrixPointX 의 값 즉 키가 포함되어 있지 않다면,
                    {
                    ...
                    try
                    {
                        ...
                        if (((OriginData.nB[i] == pMyParam.Die_Grade) || (OriginData.nB[i] == pMyParam.Die_GradeS)) && (OriginPoint[poTrans].Status == 0))
                        {
                            ...
                            */
                            ...
                            // 12.09 Test 진행 중.
                            System.Drawing.Point _poTrans = new System.Drawing.Point(); // 현재 인덱스에서 상/하 인덱스 변환 값을 가져오기 위한 변수.
                            Found_Die_Info tempDie = new Found_Die_Info();              // Model Finder 에서 이미지 찾은 상/하 인덱스가 가진 데이터를 가져오기 위한 변수
                            _poTrans.X = poTrans.X;
                            _poTrans.Y = poTrans.Y + 1;
                            if(OriginPoint[_poTrans].Status == 1)
                            {
                                pMyContext.Found_Die.TryGetValue(_poTrans, out tempDie);
                                //VMDie_CenterXW = OriginPoint[_poTrans].X;
                                VMDie_CenterXW = tempDie.WX_Pos;
                                VMDie_CenterX = tempDie.X_Pos;
                                //VM_COLS.TryGetValue(_poTrans.X, out VMDie_CenterX);
                                //VM_COLSW.TryGetValue(_poTrans.X, out VMDie_CenterXW);
                            }


                            //if (VM_COLS.ContainsKey(poTrans.X) == true) // 12.09 Origin
                            else if (VM_COLS.ContainsKey(poTrans.X) == true)
                            {
                            ...
                            else
                            {
                                ...
                                {
                                    if (((poTrans.X - SearchKey) >= 0) && VM_COLS.ContainsKey(poTrans.X - SearchKey) == true)       // 현재 인덱스의 왼쪽 방향 검색
                                    {
                                    ...
                                    else if (((poTrans.X + SearchKey <= nMaxCol)) && (VM_COLS.ContainsKey(poTrans.X + SearchKey) == true))  // 현재 인덱스의 오른쪽 방향 검색
                                    {
                                        VM_COLS.TryGetValue(poTrans.X + SearchKey, out VMDie_CenterX);
                                        VMDie_CenterX -= (((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X * 1000) + pMyParam.DieWidth) / (pMyContext.MMPerPixel_X * 1000)) * SearchKey;
                                    ...
                                    /*
                                    else if (((poTrans.X - SearchKey) >= 0) && VM_COLS.ContainsKey((poTrans.X - nMaxCol) * SearchKey) == true)       // 현재 인덱스의 위쪽 방향 검색
                                    {
                                        VM_COLS.TryGetValue((poTrans.X - nMaxCol) * SearchKey, out VMDie_CenterX);
                                        VM_COLSW.TryGetValue((poTrans.X - nMaxCol) * SearchKey, out VMDie_CenterXW);
                                    }
                                    else if (((poTrans.X + SearchKey) <= nMaxCol) && VM_COLS.ContainsKey((poTrans.X + nMaxCol) * SearchKey) == true)       // 현재 인덱스의 위쪽 방향 검색
                                    {
                                        VM_COLS.TryGetValue((poTrans.X + nMaxCol) * SearchKey, out VMDie_CenterX);
                                        VM_COLSW.TryGetValue((poTrans.X + nMaxCol) * SearchKey, out VMDie_CenterXW);
                                    }
                                ...
                                #region VM X Search Extention until #5
                                /*
                                if (((poTrans.X - 1) >= 0) && (VM_COLS.ContainsKey(poTrans.X - 1) == true))
                                {
                                    VM_COLS.TryGetValue(poTrans.X - 1, out VMDie_CenterX);
                                    VMDie_CenterX += (((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X * 1000) + pMyParam.DieWidth) / (pMyContext.MMPerPixel_X * 1000));

                                    VM_COLSW.TryGetValue(poTrans.X - 1, out VMDie_CenterXW);
                                    VMDie_CenterXW += ((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X) + (pMyParam.DieWidth / 1000));
                                }
                                else if(((poTrans.X + 1 <= nMaxCol)) && (VM_COLS.ContainsKey(poTrans.X + 1) == true))
                                {
                                    VM_COLS.TryGetValue(poTrans.X + 1, out VMDie_CenterX);
                                    VMDie_CenterX -= (((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X * 1000) + pMyParam.DieWidth) / (pMyContext.MMPerPixel_X * 1000));
                                ...
                                // 08.28 Insert Start
                                else if(((poTrans.X - 2) >= 0) && (VM_COLS.ContainsKey(poTrans.X - 2) == true))
                                {
                                    VM_COLS.TryGetValue(poTrans.X - 2, out VMDie_CenterX);
                                    VMDie_CenterX += (((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X * 1000) + pMyParam.DieWidth) / (pMyContext.MMPerPixel_X * 1000)) * 2;

                                    VM_COLSW.TryGetValue(poTrans.X - 2, out VMDie_CenterXW);
                                    VMDie_CenterXW += ((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X) + (pMyParam.DieWidth / 1000)) * 2;
                                }
                                else if(((poTrans.X + 2 <= nMaxCol)) && (VM_COLS.ContainsKey(poTrans.X + 2) == true))
                                {
                                    VM_COLS.TryGetValue(poTrans.X + 2, out VMDie_CenterX);
                                    VMDie_CenterX -= (((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X * 1000) + pMyParam.DieWidth) / (pMyContext.MMPerPixel_X * 1000)) * 2;
                                ...
                                else if (((poTrans.X - 3) >= 0) && (VM_COLS.ContainsKey(poTrans.X - 3) == true))
                                {
                                    VM_COLS.TryGetValue(poTrans.X - 3, out VMDie_CenterX);
                                    VMDie_CenterX += (((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X * 1000) + pMyParam.DieWidth) / (pMyContext.MMPerPixel_X * 1000)) * 3;

                                    VM_COLSW.TryGetValue(poTrans.X - 2, out VMDie_CenterXW);
                                    VMDie_CenterXW += ((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X) + (pMyParam.DieWidth / 1000)) * 3;
                                }
                                else if (((poTrans.X + 3 <= nMaxCol)) && (VM_COLS.ContainsKey(poTrans.X + 3) == true))
                                {
                                    VM_COLS.TryGetValue(poTrans.X + 3, out VMDie_CenterX);
                                    VMDie_CenterX -= (((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X * 1000) + pMyParam.DieWidth) / (pMyContext.MMPerPixel_X * 1000)) * 3;

                                    VM_COLSW.TryGetValue(poTrans.X + 3, out VMDie_CenterXW);
                                    VMDie_CenterXW -= ((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X) + (pMyParam.DieWidth / 1000)) * 3;
                                }
                                else if (((poTrans.X - 4) >= 0) && (VM_COLS.ContainsKey(poTrans.X - 4) == true))
                                {
                                    VM_COLS.TryGetValue(poTrans.X - 4, out VMDie_CenterX);
                                    VMDie_CenterX += (((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X * 1000) + pMyParam.DieWidth) / (pMyContext.MMPerPixel_X * 1000)) * 4;

                                    VM_COLSW.TryGetValue(poTrans.X - 2, out VMDie_CenterXW);
                                    VMDie_CenterXW += ((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X) + (pMyParam.DieWidth / 1000)) * 4;
                                }
                                else if (((poTrans.X + 4 <= nMaxCol)) && (VM_COLS.ContainsKey(poTrans.X + 4) == true))
                                {
                                    VM_COLS.TryGetValue(poTrans.X + 4, out VMDie_CenterX);
                                    VMDie_CenterX -= (((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X * 1000) + pMyParam.DieWidth) / (pMyContext.MMPerPixel_X * 1000)) * 4;

                                    VM_COLSW.TryGetValue(poTrans.X + 4, out VMDie_CenterXW);
                                    VMDie_CenterXW -= ((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X) + (pMyParam.DieWidth / 1000)) * 4;
                                }
                                else if (((poTrans.X - 5) >= 0) && (VM_COLS.ContainsKey(poTrans.X - 5) == true))
                                {
                                    VM_COLS.TryGetValue(poTrans.X - 5, out VMDie_CenterX);
                                    VMDie_CenterX += (((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X * 1000) + pMyParam.DieWidth) / (pMyContext.MMPerPixel_X * 1000)) * 5;

                                    VM_COLSW.TryGetValue(poTrans.X - 2, out VMDie_CenterXW);
                                    VMDie_CenterXW += ((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X) + (pMyParam.DieWidth / 1000)) * 5;
                                }
                                else if (((poTrans.X + 5 <= nMaxCol)) && (VM_COLS.ContainsKey(poTrans.X + 5) == true))
                                {
                                    VM_COLS.TryGetValue(poTrans.X + 5, out VMDie_CenterX);
                                    VMDie_CenterX -= (((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X * 1000) + pMyParam.DieWidth) / (pMyContext.MMPerPixel_X * 1000)) * 5;

                                    VM_COLSW.TryGetValue(poTrans.X + 5, out VMDie_CenterXW);
                                    VMDie_CenterXW -= ((pMyParam.Scrib_X * 2) * (pMyContext.MMPerPixel_X) + (pMyParam.DieWidth / 1000)) * 5;
                                }
                                // 08.28 Insert End
                                ...
                                {
                                    VMDie_CenterX = ScreenCenterX + dist_X + (pMyContext.CenterOffsetXmm / pMyContext.MMPerPixel_X);
                                    VMDie_CenterXW = VMdataList[i].XP + pMyContext.CenterOffsetXmm;

                                    Logging.PrintLog((int)ELogType.Error, $"Virtual Mapping Error - X:{poTrans.X}");
                                }
                                */
                                #endregion VM X Search Extention until #5
                            ...
                            {
                                /*
                                // poTrans.Y is 0, Debugging Code
                                if (poTrans.Y == 0)
                                {
                                    Debug.WriteLine($"Y:{poTrans.Y}");
                                    int AddKey = 0;
                                    while(AddKey < nMaxRow)
                                    {
                                        if (VM_ROWS.ContainsKey(poTrans.Y + AddKey) == true)
                                        {
                                            double pValue = 0.0;
                                            VM_ROWSW.TryGetValue(AddKey, out pValue);
                                            Debug.WriteLine($"Key:{AddKey}, value:{pValue}");
                                            break;
                                        }
                                        AddKey++;
                                    }
                                ...
                                */
                                ...
                                while(SearchKey < (nMaxRow + 1))
                                {
                                    ...
                                    else if(((poTrans.Y + SearchKey) <= nMaxRow) && VM_ROWS.ContainsKey(poTrans.Y + SearchKey) == true)
                                    {
                                        VM_ROWS.TryGetValue(poTrans.Y + SearchKey, out VMDie_CenterY);
                                        VMDie_CenterY -= (((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y * 1000) + pMyParam.DieHeight) / (pMyContext.MMPerPixel_Y * 1000)) * SearchKey;
                                ...
                                #region VM Y Search Extention until #5

                                /*
                                if (((poTrans.Y - 1) >= 0) && VM_ROWS.ContainsKey(poTrans.Y - 1) == true)
                                {
                                    VM_ROWS.TryGetValue(poTrans.Y - 1, out VMDie_CenterY);
                                    VMDie_CenterY += (((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y * 1000) + pMyParam.DieHeight) / (pMyContext.MMPerPixel_Y * 1000));

                                    VM_ROWSW.TryGetValue(poTrans.Y - 1, out VMDie_CenterYW);
                                    VMDie_CenterYW += ((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y) + (pMyParam.DieHeight / 1000));
                                }
                                else if(((poTrans.Y + 1) <= nMaxRow) && VM_ROWS.ContainsKey(poTrans.Y + 1) == true)
                                {
                                    VM_ROWS.TryGetValue(poTrans.Y + 1, out VMDie_CenterY);
                                    VMDie_CenterY -= (((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y * 1000) + pMyParam.DieHeight) / (pMyContext.MMPerPixel_Y * 1000));
                                ...
                                // 08.28 Insert Start
                                else if (((poTrans.Y - 2) >= 0) && VM_ROWS.ContainsKey(poTrans.Y - 2) == true)
                                {
                                    VM_ROWS.TryGetValue(poTrans.Y - 2, out VMDie_CenterY);
                                    VMDie_CenterY += (((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y * 1000) + pMyParam.DieHeight) / (pMyContext.MMPerPixel_Y * 1000)) * 2;

                                    VM_ROWSW.TryGetValue(poTrans.Y - 2, out VMDie_CenterYW);
                                    VMDie_CenterYW += ((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y) + (pMyParam.DieHeight / 1000)) * 2;
                                }
                                else if(((poTrans.Y + 2) <= nMaxRow) && VM_ROWS.ContainsKey(poTrans.Y + 2) == true)
                                {
                                    VM_ROWS.TryGetValue(poTrans.Y + 2, out VMDie_CenterY);
                                    VMDie_CenterY -= (((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y * 1000) + pMyParam.DieHeight) / (pMyContext.MMPerPixel_Y * 1000)) * 2;
                                ...
                                else if (((poTrans.Y - 3) >= 0) && VM_ROWS.ContainsKey(poTrans.Y - 3) == true)
                                {
                                    VM_ROWS.TryGetValue(poTrans.Y - 3, out VMDie_CenterY);
                                    VMDie_CenterY += (((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y * 1000) + pMyParam.DieHeight) / (pMyContext.MMPerPixel_Y * 1000)) * 3;

                                    VM_ROWSW.TryGetValue(poTrans.Y - 3, out VMDie_CenterYW);
                                    VMDie_CenterYW += ((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y) + (pMyParam.DieHeight / 1000)) * 3;
                                }
                                else if(((poTrans.Y + 3) <= nMaxRow) && VM_ROWS.ContainsKey(poTrans.Y + 3) == true)
                                {
                                    VM_ROWS.TryGetValue(poTrans.Y + 3, out VMDie_CenterY);
                                    VMDie_CenterY -= (((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y * 1000) + pMyParam.DieHeight) / (pMyContext.MMPerPixel_Y * 1000)) * 3;
                                ...
                                else if (((poTrans.Y - 4) >= 0) && VM_ROWS.ContainsKey(poTrans.Y - 4) == true)
                                {
                                    VM_ROWS.TryGetValue(poTrans.Y - 4, out VMDie_CenterY);
                                    VMDie_CenterY += (((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y * 1000) + pMyParam.DieHeight) / (pMyContext.MMPerPixel_Y * 1000)) * 4;

                                    VM_ROWSW.TryGetValue(poTrans.Y - 4, out VMDie_CenterYW);
                                    VMDie_CenterYW += ((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y) + (pMyParam.DieHeight / 1000)) * 4;
                                }
                                else if(((poTrans.Y + 4) <= nMaxRow) && VM_ROWS.ContainsKey(poTrans.Y + 4) == true)
                                {
                                    VM_ROWS.TryGetValue(poTrans.Y + 4, out VMDie_CenterY);
                                    VMDie_CenterY -= (((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y * 1000) + pMyParam.DieHeight) / (pMyContext.MMPerPixel_Y * 1000)) * 4;
                                ...
                                else if (((poTrans.Y - 5) >= 0) && VM_ROWS.ContainsKey(poTrans.Y - 5) == true)
                                {
                                    VM_ROWS.TryGetValue(poTrans.Y - 5, out VMDie_CenterY);
                                    VMDie_CenterY += (((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y * 1000) + pMyParam.DieHeight) / (pMyContext.MMPerPixel_Y * 1000)) * 5;

                                    VM_ROWSW.TryGetValue(poTrans.Y - 5, out VMDie_CenterYW);
                                    VMDie_CenterYW += ((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y) + (pMyParam.DieHeight / 1000)) * 5;
                                }
                                else if(((poTrans.Y + 5) <= nMaxRow) && VM_ROWS.ContainsKey(poTrans.Y + 5) == true)
                                {
                                    VM_ROWS.TryGetValue(poTrans.Y + 5, out VMDie_CenterY);
                                    VMDie_CenterY -= (((pMyParam.Scrib_Y * 2) * (pMyContext.MMPerPixel_Y * 1000) + pMyParam.DieHeight) / (pMyContext.MMPerPixel_Y * 1000)) * 5;
                                ...
                                {
                                    VMDie_CenterY = ScreenCenterY + dist_Y + (pMyContext.CenterOffsetYmm / pMyContext.MMPerPixel_Y);
                                    VMDie_CenterYW = VMdataList[i].YP + pMyContext.CenterOffsetYmm;

                                    Logging.PrintLog((int)ELogType.Error, $"Virtual Mapping Error - Y:{poTrans.Y}");
                                }
                                */

                                //Debug.WriteLine($"Y Index:{poTrans.Y}, Create Virtual Map Y-Postion Pixel:{VMDie_CenterY}, World:{VMDie_CenterYW}");
                                #endregion VM Search Extention until #5
                            }
                            ...
                            //VM_Die_Info.WX_Pos = value.X = VMdataList[i].XP;   //08.12 주석처리.
                            //VM_Die_Info.WY_Pos = value.Y = VMdataList[i].YP;  // 08.12 주석 처리.
            ...
            switch ((EStep)Step)
            {
                case EStep.Grab:
                    ...
                    {   // change code 12.23 create the BinaryImage
                        ...
                        GrabToThresholdImage = new Mat(Context.ResultImage.Size(), MatType.CV_8UC1);    // 11.22
                    }
                    ...
                    if (pMyParam.WaferThreshold != 0)               // 0이 아니라면 Model Finder에서 모델을 찾기 위한 이미지는 WaferThreshold 값이 적용된 이미지사용.
                        Cv2.Threshold(GrayImage, GrabToThresholdImage, pMyParam.WaferThreshold, 255, ThresholdTypes.Binary);  // 11.22
                    else
                        GrabToThresholdImage = GrayImage.Clone();   // WaferThreshold 값이 0이면, 검사 데이터에 넘기는 이미지데이터는 원본 데이터로 전달.

                    // 11.22 Insert
                    if(pMyParam.TeachingImage == true)
                        Context.ResultImage = GrabToThresholdImage;    // 11.22 메인 화면에 표시되는 데이터를 WaferThreshold 값이 적용된 이미지 파일.
                    else
                    {
                        // 11.25 Insert
                        if ((pMyParam.WaferTHEnable == true) && (pMyParam.WaferThreshold != 0) && (pMyParam.Mophology == false))
                        {
                            Context.ResultImage = GrabToThresholdImage;                 // Main UI 화면에 보여 지는 이미지 (WaferThreshold 적용된 이미지)
                            pMyContext.CrobImage = GrabToThresholdImage.Clone();        // Lef/Right View에 보여지는 이미지 및 Crob 하기 위한 이미지 복사
                        }
                        else
                        {
                            Context.ResultImage = GrayImage;
                    ...
                    if (pMyParam.Mophology)
                    {
                        ...
                        */
                        ...
                        //Cv2.MorphologyEx(MophologyImage, MophologyImage, MorphTypes.Open, element);   // Origin
                        ...
                        //Cv2.MorphologyEx(MophologyImage, MophologyImage, MorphTypes.Close, element, null, pMyParam.Mophology_Iteration); // iteration is 3, Insert 08.28

                        // 02.15 Insert Start
                        if ((pMyParam.MophologyImage == true) && (pMyParam.MapCheck == true))   // Mophology를 사용하고, Map파일이 없는 경우
                        {
                    ...
                    // 11.25 주석 처리하고, pMyParam.WaferTHEnable 의 조건에 따라 pMyContext.CrobImage에 GrayImage 또는 GrabToThresholdImage 를 선택하여 넣도록 수정함.
                    //pMyContext.CrobImage = GrayImage.Clone();       // Crob 하기 위한 이미지 복사
                ...
                case EStep.Calibration:
                    ...
                    if (pMyParam.CalCheck)  // Calibration Option Always True 
                    {
                        ...
                        /*
                        // Before 06.14 Origin Code
                        pMyContext.MMPerPixel_X = calibParam.MMPerPixel_LX;     // 1Pixel per mm //0.0484539718412904
                        pMyContext.MMPerPixel_Y = calibParam.MMPerPixel_LY;     // 1Pixel Per mm //0.0488501543569413
                        */
                ...
                case EStep.LineAngle:
                    ...
                    if (pMyParam.OrientSearch == false)     // OrientSearch 사용하지 않음.
                    {
                        ...
                        */
                        ...
                        if ((calAngle < -pMyParam.Tolerance_Rotate) || (calAngle > pMyParam.Tolerance_Rotate))       // 0.4
                        {
                            ...
                            // 11.04 Insert
                            //CustomMessageBox.Show($"Wafer Angle Error", $"{pMyParam.ProcessName}-Wafer Angle Error:" + "\n" + $"Wafer Angle:{Math.Abs(calAngle)} > {pMyParam.Tolerance_Rotate}",
                    ...
                    if ((pMyContext.AngleFoundResult == EVisionResultType.OK) && (pMyParam.SaveWaferImage == true) && (pMyParam.MapCheck == true))      // Unsued MapFile 조건
                    {
                        ...
                        //Cv2.ImWrite(strFileName, Context.ResultImage);    // Origin 11.22 주석 처리.
                ...
                case EStep.CircleFind:
                    try
                    {
                        ...
                        if ((nGet == 0) && (dGetRansacRadius != 0))       // 정상적으로 Circle을 찾았다면, Radius 가 0이 아니라면
                        {
                            ...
                            // 05.23 Circle 중심이 화면 중심으로 이동해야 offset 값으로 넘기는 계산이 잘못되어 있어, 주석 처리함.
                            #region OLD_Circle_Move_Offset_Start
                            /*
                            // 01.31 Insert Start
                            ...
                            // 01.31 Insert End
                            */
                            #endregion OLD_Circle_Move_Offset_End

                            #region New_Circle_Move_Offset_Start
                            // 05.23 Circle 중심이 화면 중심으로 이동해야 하는 Offset 값.
                            // Calibration 적용된 이미지의 경우, 화면 중심이 (0,0) 이며, X축은 Negative 방향이 왼쪽. Positive 방향이 오른쪽.
                            // Calibration 적용된 이미지의 경우, 화면 중심이 (0,0) 이며, Y축은 Negative 방향이 위쪽. Positive 방향이 아래쪽.
                            // Circle 중심 값 기준으로 Offset 값의 부호를 반대로 적용.
                            if (pMyContext.dFoundCenterXWorld < 0)
                                ...
                                //pMyContext.CenterOffsetXmm = pMyContext.dFoundCenterXWorld;     // 10.31
                            ...
                            {
                                //Logging.PrintErrLog((int)ELogType.Error, "ScreenCenter To Circle Center Fail! - {0} > {1}: ReturenCode:{2}",
                                    //"Dist-X:"+ Math.Abs(pMyContext.CenterOffsetXmm).ToString() + ", Dist-Y:" + Math.Abs(pMyContext.CenterOffsetYmm).ToString(), pMyParam.ScreenCenterToCircleCenter.ToString(), nGet);
                ...
                case EStep.End:
                    ...
                    //    Debug.WriteLine($"PASS-Elapsed Time is:{stopwatch.ElapsedMilliseconds}ms");
                    //    //pMyContext.ProcessTime = stopwatch.ElapsedMilliseconds;
                    //    //pMyContext.ProcessTime = String.Format($"PASS-Elapsed Time is:{stopwatch.ElapsedMilliseconds}ms");
                    //}
